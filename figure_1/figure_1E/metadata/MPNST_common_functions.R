...
### Alex Stein: Adapted heatmap function because of buggy label length

sc_asCN_heatmap2 <- function(CN_mtx, hclust = FALSE, km = NULL, row_split = NULL, probes,
                             row_ann = NULL, column_title = "Single Cell Copy Number Heatmap",
                             title = "Allele-specific copy number state",
                             colour_scheme = c("royalblue3","skyblue2","grey80","white","gold1","khaki1",
                                               "darkorange3","darkorange1","orange","red4","red","orangered2","purple4")) {
  
  show_raw_dend <- TRUE
  if (!is.null(km)) { hclust <- TRUE; show_raw_dend <- FALSE }
  
  CN_mtx[CN_mtx < 0]  <- 0
  CN_mtx[CN_mtx > 60] <- 60
  
  names(colour_scheme) <- c(0,10,20,21,30,31,40,41,42,50,51,52,60)
  
  # --- IMPORTANT: define 'at' and matching 'labels' ---
  ...
  ha_column <- HeatmapAnnotation(odd = anno_empty(border = FALSE),
                                 even = anno_empty(border = FALSE))
  ...
  # keep the rest of your decorate_heatmap_body / decorate_annotation code below...
}

### And once more but only minimal changes
