...
library(ape, lib.loc = "/camp/lab/vanloop/working/yanh/R/library/")
library(phangorn, lib.loc = "/camp/lab/vanloop/working/yanh/R/library/")
library(ggtree, lib.loc = "/camp/lab/vanloop/working/yanh/R/library/")
library(Seurat, lib.loc = "/camp/lab/vanloop/working/yanh/R/library/")
...
output.dir = paste0("/camp/project/proj-vanloo/analyses/hyan/mpnst/10X_spatial/results/inferCNV_samples_seg_10X_DLP/")
system(paste0("mkdir -p ", output.dir))
setwd(output.dir)

#Load segments from 10X DNA
all_10X_DLP.dir = paste0("/camp/project/proj-vanloo/analyses/hyan/mpnst/10X_DLP/results/ASCAT.sc/")
...
#   
# }
all_probes <- readRDS(paste0("../../../10X_DLP/results/ASCAT.sc/MPNST_all_probes_mpcf_5.rds"))
all_chr_probes <- readRDS(paste0("../../../10X_DLP/results/ASCAT.sc/MPNST_all_chr_probes_mpcf_5.rds"))
...
for (sample in pairs[1:7]) {
  ...
  if (T & !single_spot) {
    medicc_input_name <- paste0("MPNST_visium_clusters_", gsub(".*_", "", sample),suffix)
    medicc_visium_tree <- read.tree(paste0(output.dir, medicc_input_name, "/", medicc_input_name, "_final_tree.new"))
  ...
  #Make randomised plot
  if (F & !single_spot) {
    set.seed(111)
    rng_order <- base::sample(1:nrow(subclone_locations), replace = F)
    subclone_locations_rng <- subclone_locations[,1:2] %>% add_column(imagecol = subclone_locations$imagecol[rng_order], imagerow = subclone_locations$imagerow[rng_order])
    ...
    edges <- data.frame(parent = all[medicc_visium_tree[["edge"]][,1]],
                        child = all[medicc_visium_tree[["edge"]][,2]],
                        length = medicc_visium_tree[["edge.length"]]) %>% left_join(subclone_locations_rng %>% select(cluster_name, imagecol, imagerow), by = c("child" = "cluster_name")) %>%
      filter(str_detect(child, "diploid", negate = T)) %>% mutate(edge = 1:nrow(.)) %>% rename(coord.x = imagecol, coord.y = imagerow)
    ...
    edges_ggplot <- rbind(edges_adj[,c(2,4,5,6)] %>% mutate(direction = "end"), 
                          edges_adj[,c(1,6)] %>% left_join(edges_adj[,c(2,4,5)], by = c("parent" = "child")) %>% select(1,3,4,2) %>% rename(child = parent) %>% mutate(direction = "start")) %>% 
      mutate(type = ifelse(str_detect(child, gsub(".*_", "", sample)), child, ifelse(str_detect(child, "MRCA"), "MRCA", "Node"))) %>%filter(str_detect(child, "root", negate = T))
    spot_colors <- c("navy", "grey", hcl(h = seq(15, 375, length = length(unique(MPNST_sp_markers[[which(visium_samples == gsub(".*_", "", sample))]]$subclone)) + 1), l = 65, c = 100) %>% head(-1))
    names(spot_colors) <- c("MRCA", "Node", sort(unique(MPNST_sp_markers[[which(visium_samples == gsub(".*_", "", sample))]]$subclone)))
    
    #Plot points on image
    png(filename = paste0("MPNST_", sample, "_spatialplot_subclones_tree_rng",suffix,".png"), width = 2000, height = 2000, res = 200)
    print(SpatialPlot(MPNST_sp_markers[[which(visium_samples == gsub(".*_", "", sample))]], group.by = "subclone", pt.size.factor = 1.2) + labs(color = "Type") + 
            geom_point(data = edges_ggplot, mapping = aes(x = coord.x, y = coord.y, color = type), shape = 16, size = 10, inherit.aes = F) + scale_color_manual(values = spot_colors) + 
            geom_path(data = edges_ggplot %>% arrange(edge, desc(direction)), mapping = aes(x = coord.x, y = coord.y, group = edge), arrow = arrow(length=unit(20,"pt")), colour = "navy", size = 1, inherit.aes = F) + 
            theme(legend.position = "right"))
  ...
  #Plot hclust tree
  if (F & !single_spot) {
    hclust_tree <- MPNST_hclust_cor_ward %>% as.phylo
    for (c in 1:length(merge_cluster_ids)) {
      hclust_tree <- drop.tip(hclust_tree, merge_cluster_ids[[c]][2:length(merge_cluster_ids[[c]])])
      hclust_tree <- hclust_tree %>% as_tibble() %>% mutate(label = ifelse(is.na(label), paste0("internal_", row_number()), ifelse(label == merge_cluster_ids[[c]][1], paste0(gsub(".*_", "", sample), "_Cluster_", c),label))) %>% as.phylo()
    }
    hclust_tree <- bind.tree(read.tree(text = "(:1);"), hclust_tree, where = 1)
    
    
    # hclust_tree <- hclust_tree %>% as_tibble() %>% 
    #   mutate(parent = ifelse(row_number() == length(merge_cluster_ids)+1, max(parent)+1, parent), label = ifelse(row_number() == length(merge_cluster_ids)+1, "MRCA", label)) %>% 
    #   add_row(parent = max(.$parent), node = max(.$parent), branch.length = NA, label = "root") %>% as.phylo()
    subclone_locations <- readRDS(paste0("MPNST_sp_",gsub(".*_", "", sample),"_subclone_locations_", K, ".rds"))
    
    #Generate location of intermediate nodes
    all <- c(hclust_tree[["tip.label"]], "root", hclust_tree[["node.label"]][-1])
    nodes <- hclust_tree[["node.label"]][-1]
    edges <- data.frame(parent = all[hclust_tree[["edge"]][,1]],
                        child = all[hclust_tree[["edge"]][,2]],
                        length = hclust_tree[["edge.length"]]) %>% left_join(subclone_locations %>% select(cluster_name, imagecol, imagerow), by = c("child" = "cluster_name")) %>%
      filter(str_detect(child, "diploid", negate = T)) %>% mutate(edge = 1:nrow(.)) %>% rename(coord.x = imagecol, coord.y = imagerow) 
    ...
    #Rename to highest node (first row) to MRCA
    edges_adj <- edges_adj %>% mutate(parent = str_replace(parent, edges_adj[1,"child"], "MRCA"))
    edges_adj[1,"child"] <- "MRCA"
    
    # MRCA_nodename = edges_adj %>% filter(str_detect(child, "Cluster", negate = T)) %>% .[1,"child"]
    # edges_adj <- edges_adj %>% mutate(parent = str_replace(parent, MRCA_nodename, "MRCA"), child = str_replace(child, MRCA_nodename, "MRCA"))

    edges_ggplot <- rbind(edges_adj[,c(2,4,5,6)] %>% mutate(direction = "end"), 
                          edges_adj[,c(1,6)] %>% left_join(edges_adj[,c(2,4,5)], by = c("parent" = "child")) %>% select(1,3,4,2) %>% rename(child = parent) %>% mutate(direction = "start")) %>% 
      mutate(type = ifelse(str_detect(child, "Cluster"), child, ifelse(str_detect(child, "MRCA"), "MRCA", "Node"))) %>%filter(str_detect(child, "root", negate = T))
    spot_colors <- c("navy", "grey", hcl(h = seq(15, 375, length = length(unique(edges_ggplot$type))-2 + 1), l = 65, c = 100) %>% head(-1))
    
    #Plot points on image
    png(filename = paste0("MPNST_", sample, "_spatialplot_subclones_", K, "_hclust_tree.png"), width = 2000, height = 2000, res = 200)
    print(SpatialPlot(MPNST_sp_markers[[which(visium_samples == gsub(".*_", "", sample))]], group.by = "infercnv_subclone", pt.size.factor = 1.2) + labs(color = "Type") + 
            geom_point(data = edges_ggplot, mapping = aes(x = coord.x, y = coord.y, color = type), shape = 16, size = 10, inherit.aes = F) + scale_color_manual(values = spot_colors) + 
            geom_path(data = edges_ggplot %>% arrange(edge, desc(direction)), mapping = aes(x = coord.x, y = coord.y, group = edge), arrow = arrow(length=unit(20,"pt")), colour = "navy", size = 1, inherit.aes = F) + 
            theme(legend.position = "right"))
    dev.off()
